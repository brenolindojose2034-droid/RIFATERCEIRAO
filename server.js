...
let precoCota = 10; // Valor padrão inicial

app.get('/status-rifa', async (req, res) => {
    // Limpeza automática de 10 min
...
app.post('/gerar-pagamento', async (req, res) => {
    ...
    } catch (e) { res.status(500).json({ erro: "Erro no MP" }); }
});
...
app.post('/admin/acao', async (req, res) => {
    ...
    if (acao === 'limpar_tudo') await supabase.from('rifas').delete().neq('id', 0);
    res.json({ ok: true });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log("🚀 Motor Ultra ON!"));